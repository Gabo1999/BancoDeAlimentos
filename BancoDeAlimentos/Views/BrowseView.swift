...
struct RecentsGrid: View {
    ...
    func getRecentsUniqueOrdered() -> [TrophiesModel] {
        var recentsUniqueOrderedArray: [TrophiesModel] = []
...
struct ModelsByCategoryGrid: View {
    ...
    var body: some View {
        VStack {
            ForEach(TrophiesCategory.allCases, id: \.self) { category in
                ...
                LazyHGrid(rows: gridItemLayout, spacing: 30) {
                    ForEach(0..<items.count) { index in
                        ...
                        ItemButton(model: model) {
                            ...
                            print("BrowsserView: selected \(model.name)")
